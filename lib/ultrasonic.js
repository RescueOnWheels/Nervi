...
class Ultrasonic extends EventEmitter {
  ...
  constructor(pinTrigger, pinEcho) {
     ...
     */
    this.triggerInterval = setInterval(() => self.trigger.trigger(10, 1), 1000);
  }
  ...
  onAlert(level, tick) {
    if (level === 1) {
      ...
      /**
       * The distance towards the object, that the ultraosnic sensor is pointing at; in meters.
       */
       ...
       */
      debug(`Pulse traveled ${Math.round((distance * 2) * 100) / 100}m in ${Math.round((time) * 100) / 100}μs; Distance to the object is ${Math.round(distance * 100) / 100}m.`);
